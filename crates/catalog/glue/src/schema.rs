...
impl SchemaVisitor for GlueSchemaBuilder {
    ...
    fn primitive(&mut self, p: &iceberg::spec::PrimitiveType) -> iceberg::Result<Self::T> {
        let glue_type = match p {
            ...
            // Glue has no timestamptz column type; the Iceberg table metadata
            // retains timezone semantics, while Glue stores the catalog column
            // as the nearest timestamp type it can represent.
...
mod tests {
    ...
    fn test_schema_with_timestamptz_fields() -> Result<()> {
        ...
        let expected = vec![
            create_column("created_at", "timestamp", "1", false)?,
            create_column("updated_at", "timestamp_ns", "2", false)?,
        ];
